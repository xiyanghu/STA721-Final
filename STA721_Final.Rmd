...
```{r, eval=F, cache=TRUE}
## X matrix and scale
X = model.matrix(lm.full)[,-1]
X.scaled = scale(X)/sqrt(n-1)

## data for jags
data = list(Y = bioassay$uterus, X = X.scaled, p = p, n = n)
data$scales =  attr(X.scaled, "scaled:scale")*sqrt(n-1)
...
```{r, eval=F}
as.mcmc(jags.result$BUGSoutput$sims.matrix)
```
